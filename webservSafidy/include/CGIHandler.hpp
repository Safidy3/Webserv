...
public:
    ...
    HTTPResponse executeCGI(const std::string& scriptPath) {
        ...
        std::map<std::string, std::string> environment = 
            CGIEnvironment::buildEnvironment(request, server, scriptPath);
        
        // Execute CGI script (pass body only for POST)
        std::string bodyToPass = (request.method == "POST") ? request.body : "";
        CGIExecutor::CGIResult result = CGIExecutor::executeCGI(
            scriptPath, environment, bodyToPass, cgiTimeoutSeconds);
